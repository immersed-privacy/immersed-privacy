...
class UnityCommunication(object):
    ...
    def add_character(self, character_resource='Chars/Male1', position=None, initial_room=""):
        ...
        if position is not None:
            ...
            # to extend the initial room, we can add more rooms to the list
            # For now, we just use the custom room name if it is not a canonical room in VirtualHome
    ...
    def move_character(self, char_index, pos, rotation=None):
        """
        Move the character `char_index` to a new position, optionally setting rotation.

        :param int char_index: the index of the character you want to move
        :param list pos: the position where you want to place the character [x, y, z]
        :param list rotation: optional euler angles [pitch, yaw, roll] in degrees
        ...
        if rotation is not None:
            payload['character_rotation'] = {'x': rotation[0], 'y': rotation[1], 'z': rotation[2]}
            payload['has_rotation'] = True
        response = self.post_command(
            {'id': str(time.time()),
             'action': 'move_character',
             'stringParams': [json.dumps(payload)]
            })
        return response['success']

    def deactivate_character(self, char_index=0):
        """
        Deactivate a character's game object, making it invisible in the scene.
        This does not affect any other objects in the scene.

        :param int char_index: the index of the character to deactivate
        :return: success (bool)
        """
        response = self.post_command(
            {'id': str(time.time()),
             'action': 'deactivate_character',
             'intParams': [char_index]})
        return response['success']
    ...
    def expand_scene(self, new_graph, randomize=False, random_seed=-1, animate_character=False,
        ...
        :param bool exact_expand: use exact ID matching for graph alignment instead of sequence alignment (default True)

        :return: pair success (bool), message: (str)
        """
        config = {
            'randomize_execution': randomize, 
...
def _decode_image(img_string):
    ...
    if 'PNG' == img_bytes[1:4]:
        img_file = cv2.imdecode(np.frombuffer(img_bytes, np.uint8), cv2.IMREAD_COLOR)
    else:
        img_file = cv2.imdecode(np.frombuffer(img_bytes, np.uint8), cv2.IMREAD_ANYDEPTH+cv2.IMREAD_ANYCOLOR)
    return img_file
